...
\begin{figure}[!htb]
	...
	\caption{To classify soft cuts of arbitrary length, we repeatedly test fixed-size frame sequences. In this example we test sequences of size 6. Afterwards, the predictions given by the RNN/LSTM are merged, so that we have one prediction per frame.}
...
Concretely, the last prediction is not a prediction for the frame under consideration, but the intuition behind this is that an RNN/LSTM becomes more and more certain after having seen multiple frames of the sequence.
