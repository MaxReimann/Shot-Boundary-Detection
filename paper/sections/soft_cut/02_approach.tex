...
Therefore we decided to use a fixed size for the sequences of frames in our training data, i.e. we would only generate transitions of length ten in our training data.
...
In the end, we have six predictions per frame, which have to be merged, so that we only have one prediction per frame.
...
This is exactly, what the RNN/LSTM does, so it seemed like a good idea to put more emphasis on this part and less emphasis on the CNN part.
